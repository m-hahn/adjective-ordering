  ...
  slides.dialogue = slide({
    ...
    present_handle : function(stim) {
      ...
      for(var i=0; i< stim.context.length; i++) {
         if(i == stim.context.length-1) {
            for(j=0; j<stim.context[i].length; j++) {
              if(stim.context[i].slice(j,j+4) == "</b>") {
                foundNoun = j+5
                break
              }
            }
            console.log(stim.context[i].slice(foundNoun))
            for(j = foundNoun; j<stim.context[i].length; j++) {
               if(stim.context[i][j] == " ") {
                 break
               }
            }
            console.log(stim.context[i].slice(foundNoun,j))
            stim.context[i] = stim.context[i].slice(0,foundNoun)+" <u>"+stim.context[i].slice(foundNoun,j)+"</u> "+stim.context[i].slice(j)
         }
         stim.context[i] = stim.context[i].replace("<b>", "")
         stim.context[i] = stim.context[i].replace("</b>", "")
      ...
      questions = [] 
      isPlural = (stim.noun[stim.noun.length-1] == "s")
      if(isPlural) {
        plural = stim.noun
        demonstrative = "the"
        article = ""
        ...
        demonstrative = "the"
        article = "a"
        if(["a","e","i","o","u"].includes(stim.adjective[0])) {
           article = "an"
        }
        verb = "was"
      }
...
//      questions.push('Have they been talking about the '+stim.adjective+' '+stim.noun+'?');
