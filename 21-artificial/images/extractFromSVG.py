...
svg = open("all_spaceships.svg", "r").read()
...
rowNames = ["green-few", "green-many", "blue-few", "blue-many", "red-few", "red-many"]
...
for row, rowName in zip(rows, rowNames):
  for index, entry in zip(range(len(row)),row):
     ...
     indexEnd = modifiedInitial.index("sodipodi:docname")
     x_min = minX(entry)-10
     y_min = minY(entry)-10
     x_max = maxX(entry)+10
     y_max = maxY(entry)+10
     ...
     modifiedInitial = modifiedInitial[:indexStart]+'viewBox="'+" ".join(map(str,[x_min, y_min, lengthX, lengthY]))+'"\n   '+modifiedInitial[indexEnd:]
     print 'viewBox="'+" ".join(map(str,[x_min, y_min, x_max-x_min, y_max-y_min]))
     with open("AUTO-"+rowName+"-"+str(index)+".svg", "w") as outFile:
