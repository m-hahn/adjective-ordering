...
#agr = dataAA %>%
#  group_by(subjectiveFirst.Label) %>%
#  summarise(Mean = mean(response), CILow = ci.low.grouped.3D(response, workerid, predicate1, predicate2), CIHigh = ci.high.grouped.3D(response, workerid, predicate1, predicate2))
#dodge = position_dodge(.9)
#
##pdf('plots/alien-pairs-order-ratings.pdf')
#ggplot(agr, aes(x=subjectiveFirst.Label,y=Mean)) +
...
#agr = dataAA %>%
#  group_by(subjectiveFirst.Label) %>%
#  summarise(Mean = mean(response), CILow = ci.low.grouped.1D(response, workerid), CIHigh = ci.high.grouped.1D(response, workerid))
#dodge = position_dodge(.9)
#
##pdf('plots/alien-pairs-order-ratings.pdf')
#ggplot(agr, aes(x=subjectiveFirst.Label,y=Mean)) +
