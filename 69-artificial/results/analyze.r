...
agr = data5 %>%
  group_by(containsSubjectiveAdjective.Label, nonAlienAdjectiveIsColor.Label, inContext.Label, moreInformative) %>%
...
ggplot(agr, aes(x=containsSubjectiveAdjective.Label,y=Mean,fill=containsSubjectiveAdjective.Label)) +
  geom_bar(stat="identity",position=dodge) +
  geom_errorbar(aes(ymin=Mean-CILow,ymax=Mean+CIHigh),position=dodge,width=.25) +
  facet_wrap(~nonAlienAdjectiveIsColor.Label+inContext.Label+moreInformative)+
...
#  filter(nonAlienAdjectiveIsColor) %>%
  group_by(containsSubjectiveAdjective.Label,nonAlienAdjectiveIsColor.Label, moreInformative) %>%
...
plot = ggplot(agr, aes(x=containsSubjectiveAdjective.Label,y=Mean)) +
  geom_bar(stat="identity",position=dodge) +
  geom_errorbar(aes(ymin=Mean-CILow,ymax=Mean+CIHigh),position=dodge,width=.25) +
  facet_wrap(~nonAlienAdjectiveIsColor.Label+moreInformative)+
  ylab('Rating for Alien First') +
...
agr = data5 %>%
  group_by(containsSubjectiveAdjective.Label, moreInformative) %>%
  summarise(Mean = mean(responseAlienFirst), CILow = ci.low(responseAlienFirst), CIHigh = ci.high(responseAlienFirst))
dodge = position_dodge(.9)

plot = ggplot(agr, aes(x=containsSubjectiveAdjective.Label,y=Mean, fill=moreInformative)) +
  geom_bar(stat="identity",position=dodge) +
  geom_errorbar(aes(ymin=Mean-CILow,ymax=Mean+CIHigh),position=dodge,width=.25) +
  ylab('Rating for Alien First') +
  xlab('Type of Alien Adjective') + theme(panel.background = element_blank(), text = element_text(size=20))
...
agr = dataProduction %>%
  group_by(section, moreInformative) %>%
...
ggplot(agr, aes(x=alien, y=Mean, fill=moreInformative)) +
  geom_bar(stat="identity",position=dodge) +
  geom_errorbar(aes(ymin=Mean-CILow,ymax=Mean+CIHigh),position=dodge,width=.25) +
  facet_wrap(~section) +
