...
exp.imagesPreloaded = [];
  ...
  slides.tutorial = slide({
    ...
    start: function() { 
      ...
      tutorial_block_obj = [[a1,    a2, a3,     a4, a5], [a15, a16, a17, a18]]
      tutorial_block_subj = [[a6, a7, a8, a9, a10],  [a11, a12, a13, a14]]
      p_obj = 0
      p_subj = 0
      exp.tutorial = ""
      ...
      for(var i=0; i<4; i++) {
         ...
         if(p_obj == 2) {
            nextBlock = "s"
//            nextBlock = tutorial_block_subj[p_subj]
  //          p_subj+=1
         } else if(p_subj == 2) {
            nextBlock = "o"
         } else {
            nextBlock = _.sample(["s","o"])
         }
         exp.tutorial = exp.tutorial+nextBlock

         if(nextBlock == "o") {
            nextBlock = tutorial_block_obj[p_obj]
            p_obj++
         } else {
            nextBlock = tutorial_block_subj[p_subj]
            p_subj++
         }
         self.tutorial_functions = self.tutorial_functions.concat(nextBlock)
      }
      
//      self.tutorial_functions = [undefined, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12, a13, a14, a15, a16, a17, a18];

      self.tutorial_functions[0]();
    }, 
    button : function() {
      self.progress ++;
...
var questionnaires = _.shuffle(['subjectivity', 'disagreement', 'what_things_mean']);
exp.structure.push(questionnaires[0])
exp.structure.push(questionnaires[1])
exp.structure.push(questionnaires[2])
