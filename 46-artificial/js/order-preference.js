...
//Wow, so many spikes -- I really love this...
       $(".alien_utterance_1").html('');
        $(".alien_utterance_2").html('');
    
      $(".alien_utterance_"+(stim.alien == aliens[0] ? "1" : "2")).html('<p class=triangle-border left"> '+stim.text+'</p>');
      ...
      this.presentDialogue(this.indexInDialogue);

    },

    presentDialogue : function(index) {
       $(".alien_utterance_1").html('');
       $(".alien_utterance_2").html('');
       this.alien = (this.alien == 0 ? 1 : 0)
       console.log("ALIEN "+this.alien);
$(".alien_utterance_"+(this.alien == 0 ? "1" : "2")).html('<p class=triangle-border left"> '+this.stim.turns[index]+'</p>');
 
    },

    button : function() {
        
        if(this.indexInDialogue +1 < this.stim.turns.length) {
          this.indexInDialogue ++;
          this.presentDialogue(this.indexInDialogue);
        } else if(this.indexInDialogue + 1 == this.stim.turns.length) {
          document.getElementById("dialogue_display").style.display = 'none' //hide();
          document.getElementById("dialogue_turker").style.display = 'block' //hide();

          this.indexInDialogue ++;
          $(".question_dialogue").html('Is the ship '+adjectives[this.stim.adjective]+"?");
          exp.sliderPost = null
         } else if(this.indexInDialogue == this.stim.turns.length) {
           console.log(exp.sliderPost);
           if (exp.sliderPost != null) {
           } else {
             $(".err").show();
          ...
          document.getElementById("dialogue_aliens").style.display = 'block' //hide();

          this.indexInDialogue ++;
           $(".answers_dialogue").html('Here is how the Aliens would respond:');
         }  else {
           this.log_responses();
           _stream.apply(this); //use exp.go() if and only if there is no "present" data.
        }
    ...
    init_sliders : function() {
      utils.make_slider("#slider0_dialogue", function(event, ui) {
        exp.sliderPost = ui.value;
      });
//      console.log($("#slider0_dialogue"));
  //    console.log($("#slider0_dialogue").slider);

   //   $("#slider0_dialogue").slider.set(0.5);
      utils.make_slider("#slider0_dialogue_1", function(event, ui) {
        exp.sliderPost = ui.value;
      }, (this.stim.alien == 1 ? this.stim.answer1 : this.stim.answer2) );
      utils.make_slider("#slider0_dialogue_2", function(event, ui) {
        exp.sliderPost = ui.value;
      }, (this.stim.alien == 0 ? this.stim.answer1 : this.stim.answer2));
    ...
    log_responses : function() {
        ...
        exp.data_trials.push({
          ...
          "correct1" : this.stim.answer1,
          "correct2" : this.stim.answer2,
          "relevant_adjective_id" : this.stim.adjective,
          "slide_number" : exp.phase
        });
    },
  });
...
//      $(".man2").html(man2);
      ...
      $(".pos_1").html(exp.pos_1)
      $(".pos_2").html(exp.pos_2)
      $(".pos_3").html(exp.pos_3)
...
//      $(".high").html("\"the "+ stim.Predicate1 + " " + stim.Predicate2 + " " + "spaceship" + "\"");
    ...
    button : function() {
    	//console.log(exp.sliderPost);
      if (exp.sliderPost_1 != null || exp.sliderPost_2 != null || exp.sliderPost_3 != null) {
        this.log_responses();
        _stream.apply(this); //use exp.go() if and only if there is no "present" data.
      } else {
        $(".err").show();
    ...
    init_sliders : function() {
      utils.make_slider("#slider0_1", function(event, ui) {
        exp.sliderPost_1 = ui.value;
      });
      utils.make_slider("#slider0_2", function(event, ui) {
        exp.sliderPost_2 = ui.value;
      });
      utils.make_slider("#slider0_3", function(event, ui) {
        exp.sliderPost_3 = ui.value;
      });
...
//          "noun" : this.stim.Noun,  
          ...
          "response_1" : exp.sliderPost_1,
          "response_2" : exp.sliderPost_2,
          "response_3" : exp.sliderPost_3,
...
//        colorblind : $("#colorblind").val(),
        ...
        response_field_1 : exp.pos_1,
        response_field_2 : exp.pos_2,
        response_field_3 : exp.pos_3,
      };
      exp.go(); //use exp.go() if and only if there is no "present" data.
    }
  });
...
//   exp.structure=['speaker_choice',  'multi_slider', 'subj_info', 'thanks'];//exp.structure=['multi_slider', 'subj_info', 'thanks'];
  ...
  pos = _.shuffle(["a verb", "a noun", "an adjective"]);
  exp.pos_1 = pos[0];
  exp.pos_2 = pos[1];
  exp.pos_3 = pos[2];
